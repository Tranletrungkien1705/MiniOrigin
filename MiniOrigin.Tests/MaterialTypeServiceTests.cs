...
/// <summary>Test danh mục Nhóm vật liệu (port từ Mst_PartMaterialType): mã bắt buộc + duy nhất, tên bắt buộc, cờ hoạt động, gán sản phẩm (nhóm phải tồn tại & hoạt động), chặn xoá khi còn sản phẩm.</summary>
public class MaterialTypeServiceTests
{
    private static (AppDbContext db, IMaterialTypeService svc, SqliteConnection conn) NewSvc()
    {
        var conn = new SqliteConnection("DataSource=:memory:"); conn.Open();
        var opt = new DbContextOptionsBuilder<AppDbContext>().UseSqlite(conn).Options;
        var db = new AppDbContext(opt, new TenantContext { OrgId = TenantContext.DefaultOrgId });
        db.Database.EnsureCreated();
        return (db, new MaterialTypeService(db), conn);
    }
    ...
    public async Task Assign_RequiresExistingActiveType()
    {
        var (db, svc, conn) = NewSvc(); using (conn)
        {
            ...
            Assert.Equal(id, (await db.Products.FindAsync(prod.Id))!.MaterialTypeId);
        }
    }
    ...
    public async Task Delete_BlockedWhenProductsReference()
    {
        var (db, svc, conn) = NewSvc(); using (conn)
        {
            ...
            var prod = new Product { Code = "P1", Name = "Gạch", MaterialTypeId = id };
            db.Products.Add(prod); await db.SaveChangesAsync();
